...
import { expenses, entries, financialDataCSV, taxDeductions } from "@/db/schema";
import { z } from "zod";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // 4. Obter dedução fiscal (se for trimestral)
    let taxDeductionValue = 0;
    
    if (validParams.quarterly) {
      logger.debug(`Querying tax deduction data for quarter`, {
        source: 'backend',
        context: 'api:reports:dre',
      ...
      // Determinar o número do trimestre (1-4)
      const quarterNumber = Math.floor((startDate.getMonth()) / 3) + 1;
      
      const taxDeductionResult = await db
        .select({
      ...
      if (taxDeductionResult.length > 0) {
        taxDeductionValue = Number(taxDeductionResult[0].value);
      }
    }
    ...
    const irpj = (validParams.quarterly || validParams.annual) && resultadoBruto > 0 
               ? resultadoBruto * 0.15 
               : 0;
               
    const csll = (validParams.quarterly || validParams.annual) && resultadoBruto > 0 
               ? resultadoBruto * 0.09 
               : 0;
